...
sourceCpp(here("source","nebula_05.cpp"))
...
generate_tree <- function(n, seed) {
  ...
  tree_data <- flametree_grow(
    ...
    time = n,
    scale = c(0.6, 0.9, 0.9),
    angle = c(10, -10, -20) * sample(c(-1, 1), 1)
  ) %>%
    mutate(
      coord_x = runif(1, .4, .6) + coord_x / 8,
      coord_y = coord_y / 8
    )
  return(tree_data)
}
...
for(seed in seeds) {
  ...
  shades <- generate_shades(1, seed)
  background <- blend_shades(shades, "black", .5)
  tree_shade <- blend_shades(shades, "black", .2)
  leaf_shade <- blend_shades(shades, "black", .2)
  nebula_shades <- c(
    blend_shades(shades, "white", .8),
    blend_shades(shades, "white", .5),
    blend_shades(shades, "white", .2)
  )
  ...
  pic <- ggplot() +
    ...
    scale_size_identity() +
    coord_cartesian(xlim = c(.02, .98), ylim = c(.02, .98)) +
    NULL
